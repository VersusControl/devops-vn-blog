...
{% for s in site.data.series %}
  ...
  <section class="series-index-card">
    {% if s.cover %}
    <a class="series-index-cover" href="{{ s.url | relative_url }}" tabindex="-1" aria-hidden="true">
      <img src="{{ s.cover | relative_url }}" alt="{{ s.title }} series cover" loading="lazy">
    </a>
    {% endif %}
